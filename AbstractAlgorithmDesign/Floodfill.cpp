...
void delete_Maze(Maze** mpp) {
    if (!mpp || !(*mpp)) {
        return; // Do nothing if the maze pointer is null
    }
    ...
    for (short x = 0; x < SIZE; ++x) {
        for (short y = 0; y < SIZE; ++y) {
            delete_Node(&(**mpp).map[x][y]); // Deallocate individual nodes
        }
    }

    delete *mpp;  // Use `delete` to deallocate the Maze object
    *mpp = nullptr; // Set pointer to null to avoid dangling references
}
// old delete_maze
// void delete_Maze(Maze *mpp) {
// 	//debug statements?
